...
// GON
UniValue buildscript(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 4)
        throw runtime_error("buildscript \"addrA\" \"signA\" \"addrB\" \"signB\" \n" );
	
    std::string addrA = params[0].get_str();
    std::string signA = params[1].get_str();
    ...
    signA = DecodeBase64(signA);
    signB = DecodeBase64(signB);
    valtype vecA(signA.begin(), signA.end());
    valtype vecB(signB.begin(), signB.end());
    vecA.push_back(DATASIG_COMPACT_ECDSA);
    vecB.push_back(DATASIG_COMPACT_ECDSA);
	
    LOCK(cs_main);
	
    CScript csA = GetScriptForDestination(DecodeDestination(addrA));
    CScript csB = GetScriptForDestination(DecodeDestination(addrB));
    std::vector<unsigned char> keyidA(csA.begin() + 3, csA.begin() + 23);
    std::vector<unsigned char> keyidB(csB.begin() + 3, csB.begin() + 23);
	
    CScript script = CScript() << OP_DUP << vecB << OP_EQUALVERIFY;
    script << keyidB << OP_DATASIGVERIFY;
    script << OP_ROT << OP_ROT;
    script << OP_DUP << vecA << OP_EQUALVERIFY;
    script << keyidA << OP_DATASIGVERIFY;
    script << OP_ADD << 0x02 << OP_MOD;
    script << OP_IF << OP_DUP << OP_HASH160 << keyidB << OP_EQUALVERIFY << OP_CHECKSIG;
    script << OP_ELSE << OP_DUP << OP_HASH160 << keyidA << OP_EQUALVERIFY << OP_CHECKSIG << OP_ENDIF;
    ...
    // add script address into wallet
    pwalletMain->AddCScript(script);
    pwalletMain->SetAddressBook(innerID, "", "send");
	
    scriptmap[signA] = script;
			
    return result;
}

// GON
UniValue signtx(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error("signtx \"hex\" \"address\" \"numA\" \"signA\" \"numB\" \"signB\" \n" );
	
    std::string address = params[1].get_str();
    std::string numA = params[2].get_str();
    std::string signA = params[3].get_str();
	...
	// GON
        std::cout << "\nprevPubKey:\n" << FormatScript(prevPubKey) << std::endl;	
			
	CScript script1 = txin.scriptSig;	
	CScript script2 = CScript() << ToByteVector(numA) << vecA << ToByteVector(numB) << vecB;
		
	CScript spk = GetScriptForDestination(DecodeDestination(address));
	valtype hash(spk.begin() + 3, spk.begin() + 23);
	CKeyID keyID = CKeyID(uint160(hash));
	...
	vector<unsigned char> vchSig;
        if (!creator.CreateSig(vchSig, keyID, scriptmap[signA])) 
	{
	    std::cout << "CreateSig error. " << std::endl;
	}
        else
	{
	    scriptSigRet << vchSig;
	...
	scriptSigRet += script2;
	scriptSigRet += script1;
		
	txin.scriptSig = scriptSigRet;
	std::cout << "scriptSig:\n" << FormatScript(scriptSigRet) << std::endl;
		
	unsigned int flag = MANDATORY_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_STRICTENC | SCRIPT_VERIFY_NULLDUMMY | SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS | SCRIPT_VERIFY_CLEANSTACK | SCRIPT_VERIFY_NULLFAIL | SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY | SCRIPT_VERIFY_CHECKSEQUENCEVERIFY | SCRIPT_VERIFY_LOW_S | SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_MAY152018_OPCODES;
	// GON
        ...
        if (pickedForkId)
        {
            ...
            if (!VerifyScript(txin.scriptSig, prevPubKey, flag,
                    MutableTransactionSignatureChecker(&mergedTx, i, amount, SCRIPT_ENABLE_SIGHASH_FORKID), &serror))
            {
                TxInErrorToJSON(txin, vErrors, ScriptErrorString(serror));
            }
...
static const CRPCCommand commands[] = {
    ...
    // GON
    {"gon", "buildscript", &buildscript, true},
    {"gon", "signtx", &signtx, true},
